...
export function getSeasons(season?: string | number) {
  if (!season) return `${BASE_API_URL}seasons/?offset=50`;

  return `${BASE_API_URL}${season}/seasons/`;
}

export function getCircuit(season: string | number = "current") {
...
export function getDrivers(arg: string | number | null) {
  ...
  if (!Number.isNaN(arg) || arg === "current") return `${url}${arg}/drivers`;
